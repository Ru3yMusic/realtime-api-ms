...
export class SchemaRegistryService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      // block bootstrap — fall back to local .avsc codecs so the REST API
      // and Kafka consumers still come online. Real-time eventing may emit
      // raw Avro instead of Confluent wire format until the registry recovers.
      this.logger.error(
